...
public class ImageLoader extends com.android.volley.toolbox.ImageLoader {
	...
	private static final int HALF_FADE_IN_TIME = 125;
	...
	public ImageLoader(FragmentActivity activity) {
		super(newRequestQueue(activity), BitmapCache.getInstance(activity
	...
	public ImageContainer get(String requestUrl, ImageView imageView,
		...
		return get(requestUrl, imageView,
				mPlaceHolderDrawables.get(placeHolderIndex), mMaxImageWidth,
				mMaxImageHeight);
	}

	public ImageContainer get(String requestUrl, ImageView imageView,
			Drawable placeHolder) {
		return get(requestUrl, imageView, placeHolder, mMaxImageWidth,
				mMaxImageHeight);
	}

	public ImageContainer get(String requestUrl, ImageView imageView,
			Drawable placeHolder, int maxWidth, int maxHeight) {
		...
		String recycledImageUrl = imageContainer != null ? imageContainer
				.getRequestUrl() : null;
		...
		if (requestUrl == null || !requestUrl.equals(recycledImageUrl)) {
			...
			if (requestUrl != null) {
				...
				imageContainer = get(
						requestUrl,
						getImageListener(mResources, imageView, placeHolder,
								mFadeInImage), maxWidth, maxHeight);
	...
	private static RequestQueue newRequestQueue(Context context) {
		...
		// On HC+ use HurlStack which is based on HttpURLConnection. Otherwise
		// fall back on
		// AndroidHttpClient (based on Apache DefaultHttpClient) which should no
		// longer be used
	...
	private static void setImageBitmap(final ImageView imageView,
			final Bitmap bitmap, Resources resources, boolean fadeIn) {
		...
		if (fadeIn) {
			...
			td.startTransition(HALF_FADE_IN_TIME*2);
		} else {
	...
	/**
	 * Interface an activity can implement to provide an ImageLoader to its
	 * children fragments.
	 */
